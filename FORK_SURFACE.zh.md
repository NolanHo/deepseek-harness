...
| `frontend-static` 对内容哈希资产发 immutable 缓存头 | C | `packages/host/frontend-static/src/index.ts` 约 10 行 | dist 服务端此前对任何文件都只回 `content-type`，于是每次刷新都重新下载 shell 里带内容 hash 的 JS 与 CSS，而 `/plugins` 下的插件包早已是 immutable（线上实测：每次刷新 415 KiB + 23 KiB gzip、1,351 KiB 解压）。现在命中 Vite 内容哈希输出目录（`assets/`、`preview/`；`apps/web/vite.config.ts` 为每个产物名固定 `[hash]`）的请求带 `cache-control: public, max-age=31536000, immutable`；index 响应与稳定命名的根文件（`favicon.svg`、`manifest.webmanifest`、`preview.html`）保持原有头，重建的 bundle 或变更的 index 仍能到达浏览器。上游 `frontend-static` 没有缓存头配置项，因此这条规则是它服务函数内带标记的补丁；Electron/桌面宿主的 dist 处理（`apps/desktop-host`）保持上游的无头行为，因为本部署走 Web 面 |
