...
| 部署级 reduced-motion 默认 | C | 1 个 shell 模块 + 1 处启动调用 + 1 段 CSS | 本部署**默认无动效**：每次启动给 `<html data-reduce-motion>` 打标，`packages/client/web/src/base.css` 把动画与过渡压成一步完成。`?reduce-motion=0` 写入按浏览器的退出选择，`?reduce-motion=1` 清除它。该标记不改变浏览器全局的 `prefers-reduced-motion`，其他站点保持各自动效 |
