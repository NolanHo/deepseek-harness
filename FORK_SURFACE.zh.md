...
| `ui-chat` 折叠态 Think 摘要 | C | `ReasoningRow.tsx` 处替换一个表达式（删除上游两个取行辅助函数）+ 删 2 条 CSS 规则 + `token-format.ts` 新增 `formatExactCount` + 1 个 locale key | **保留** —— 折叠的 Think 行改为显示推文字符数（`1,234 字符` / `1,234 characters`），取代上游「已结算取首行 / 流式取末行」的文本预览，让过程扫读不再夹带推文散文；展开内容保持上游形态。`formatExactCount` 承载精确整数分组，`formatExactTokens` 委托给它；两个含 Think 行的 jsdom 规格、lifecycle-chrome 的钉住轮询、选择性压力断言，以及 19 个可比较的 `snapshots/web/**` golden 随之移动。整体采用上游 `ReasoningRow.tsx` 或其样式表会静默恢复预览与尾部钉住，删掉新增导出或 locale key 会让带标记的调用点无法编译 |
