...
| `session-format-v0-to-v1` released-v0 子代理 descriptor 兼容 | C | `src/fork/subagent-descriptor-compat.ts` + 4 个上游文件各一处标记委托 | 冻结的 released-v0 边界只接受 descriptor 版本 3，而安装的 `@deepseek-ai/dsh-subagent` 向新 payload 打版本 4，已存的 2/3 payload 又早于版本 4 新增的组装输入；该模块拥有可选成员集合、当前版本与 2/3→4 升级，`dispositions.ts`、`migration.ts`、`payload-validation.ts`、`validation.ts` 与 `tests/relationships.spec.ts` 各保留一处标记委托 |
