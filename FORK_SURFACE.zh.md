...
| 对话头部移动端精简 | C | 两个 `max-width` 媒体块（767.98px 与 560px） | 抽屉断点以下，会话头部左边距让开 frame 固定的抽屉入口（≤767.98px 时 `padding-left: 56px`）。手机宽度下头部内边距与标签节奏收紧，会话标题面包屑保留 `min-width: 88px` 底线并经 `.crumb` 省略号截断，`.headerActions` 可收缩（`flex: 0 1 auto`、`min-width: 0`，刻意不加 `overflow: hidden`——那会把 job 徽标绝对定位的菜单裁到不可见），`.headerCorner` 归零其桌面 `-16px` 右外边距——该外边距是按桌面 28px 内边距配的，而手机内边距为 12px，使角落控件在 360/412/480/560px 下都越出视口 4px。上游该模块完全没有媒体查询，整块采纳会一次性删掉手机内边距与其上所有规则；`header-phone-styles.client.spec.ts` 会拒绝这种做法。session-log 那一半随 0.1.5-rc.2 退役：上游把该控件挪进头部的 28px 省略号菜单，已不再挤占手机头部 |
