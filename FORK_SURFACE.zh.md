...
| 对话头部移动端精简 | C | 两个 `max-width` 媒体块（767.98px 与 560px） | 抽屉断点以下，会话头部左边距让开 frame 固定的抽屉入口（≤767.98px 时 `padding-left: 56px`）。手机宽度下头部内边距与标签节奏收紧、会话标题面包屑隐藏（标题此时位于侧栏抽屉里）、标签间距收窄。session-log 那一半随 0.1.5-rc.2 退役：上游把该控件挪进头部的 28px 省略号菜单，已不再挤占手机头部 |
