...
| 部署主题配色 | C | 1 个 shell 模块 + 1 个样式表 + 1 处启动调用 | 本部署默认渲染暮蓝配色：每次启动给 `<html data-dsh-theme="harbor">` 打标，`packages/client/web/src/fork/themes.css` 承载 token 覆盖——owner 对本地安装的 dsh-market「夕港 / Harbor」皮肤的蓝白重染，加 MIT 的 Dracula markdown 与语法配色。配色同时声明在带标记的根元素**与其 body** 上：上游把 alias token 声明在 `body` 与 `body[data-ds-dark-theme]`，在应用子树里会盖过只声明在根上的块，导致面板仍是上游配色。`?theme=default` 写入按浏览器的退出选择，`?theme=harbor` 清除它。上游 theme 插件只管 light/dark/system 偏好与正文字号、没有配色扩展点，因此配色放在 shell 而不是主题注册；皮肤的壁纸与运行时 hooks 不入库 |
