...
| `vitest.web.config.ts` 权限面浏览器套件排除 | C | 1 份排除名单 | Fork 决策 2026-09-05（见 FORK_CHANGES.md）：`settings-chrome`、`permission-policy-context`、`access-confirmation`、`seeded-history` 浏览器套件断言的 Permission 设置行 / 访问模式选择器 / `/permission read-only` chip 均为 fork 不再挂载的面。恢复路径见文件内注释 |
| `vitest.expected.config.ts` 沙箱形态 expected 排除 | C | 1 份排除名单 | Fork 决策 2026-09-05（见 FORK_CHANGES.md）：`subagent-inheritance.expected.e2e.ts` 与 `image-offload.expected.e2e.ts` 钉死的 read-only 委派事件、sandbox-policy prompt 上下文、拒绝输出与 `Current DSH file policy` 句均非 fork 产出。恢复路径见文件内注释 |
