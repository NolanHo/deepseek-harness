...
| `api/session-controller` `openWorkspacePath` desktop gate + `ui-chat` open-file routing | C | ~30 lines | The open RPC consults `canOpenPath()` and fails fast with `desktop unavailable`; the Chat open face routes core file surfaces to the third-party `betterSidebar` editor (`ctx.get` duck check) when installed, maps the refusal to `fileOpen.desktopUnavailable` copy, and keeps the native opener as fallback |
