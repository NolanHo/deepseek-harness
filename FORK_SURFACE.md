...
| `client/ui-layout` AppFrame mobile shell | C | import + compose | **0.1.5-rc.2 STATUS: injection lost** — upstream rewrote AppFrame/columns/service; `src/client/fork/mobile-shell.tsx` is in tree but unreferenced; re-apply the import + composition on the new structure. | The regime hook, drawer chrome, and details sheet live in `src/client/fork/mobile-shell.tsx`; AppFrame's diff is the import line plus composition slots |
