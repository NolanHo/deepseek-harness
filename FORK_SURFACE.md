...
| `vitest.web.config.ts` permission-surface browser excludes | C | 1 exclude list | Fork decision 2026-09-05 (FORK_CHANGES.md): `settings-chrome`, `permission-policy-context`, `access-confirmation`, and `seeded-history` browser suites assert the Permission settings row / Access-mode picker / `/permission read-only` chip the fork no longer mounts. Restore path in the in-file comment |
| `vitest.expected.config.ts` sandbox-shaped expected excludes | C | 1 exclude list | Fork decision 2026-09-05 (FORK_CHANGES.md): `subagent-inheritance.expected.e2e.ts` and `image-offload.expected.e2e.ts` pin the read-only delegation events, sandbox-policy prompt context, denial output, and `Current DSH file policy` sentence the fork no longer produces. Restore path in the in-file comment |
