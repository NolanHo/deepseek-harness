...
| `ui-chat` ChatView reflow-stable scroll anchor | C | import + 1 callback arm in ChatView | `chat/fork/scroll-anchor.ts` re-asserts the held reader row on every flow-column resize (fold collapse, image loads, disclosures), with ledger bookkeeping so reader-input attribution stays correct; anchored row hidden falls back to the nearest surviving visible row above |
