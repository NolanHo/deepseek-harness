...
The Client's initial `follow`, reconnect openings, and `loadOlder()` request at least 50 append-origin `user/message` and `assistant/message` events and cross at least two `turn/start` events, including the partial Turn at the loaded window's beginning. Steering does not add a Turn boundary, and intervening events accompany the page without counting toward the message budget. The optional `turnWindow` on page and follow requests supplies these minima under `maxMessages`. Every page is Turn-aligned whatever the request: the walk stops at the first Turn start satisfying both minima, or once it has counted the request's `maxMessages` messages, and a stop inside a Turn widens back to that Turn's own `turn/start`. `maxMessages` is therefore a floor, not a hard cap: a page may exceed it by the remainder of the Turn it starts in. Only a walk that reaches the log head before any Turn start stops there and reports the history exhausted. Requests without `turnWindow` are Turn-aligned by the same rule. `loadThrough()` uses the same rules with a 200-message minimum per page; the 500-message floor does not limit the whole jump.
