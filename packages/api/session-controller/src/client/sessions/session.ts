...
/** Messages requested per history page. Kept small because the host read
 *  cost scales with the page's event count (decode is CPU-bound): 8 turns of
 *  a tool-dense agent conversation read ~90k events instead of the ~270k a
 *  25-turn page needs, cutting the cold-open decode and the client fold to
 *  about a third. Older turns page in through the same load-older path. */
