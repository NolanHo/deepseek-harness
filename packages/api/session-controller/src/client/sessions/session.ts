...
/** Minimum message count for ordinary history windows. Kept small because the
 *  host read cost scales with the page's event count (decode is CPU-bound): at
 *  this floor a tool-dense agent conversation reads ~90k events instead of the
 *  ~270k a 25-turn window needs, cutting the cold-open decode and the client
 *  fold to about a third. Older turns page in through the same load-older path. */
