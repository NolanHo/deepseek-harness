...
export class SessionController extends TypertRemoteService {
  ...
  constructor(ctx: Context, config: Config, internals: SessionControllerInternals = {}) {
    ...
    // Fork patch (FORK_SURFACE.md): the rewrite hold's relay guard — upstream
    // announces every disposal, while the fork's in-place rewrite tears its
    // Agent down and rebuilds it at the same id.
  ...
  async openWorkspacePath(
    ...
    try {
      // Fork patch (FORK_SURFACE.md): the open RPC consults canOpenPath() and fails
      // fast, before any filesystem resolution, when no native desktop can take the path.
      if (!this.canOpenPath()) {
