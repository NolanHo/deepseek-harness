...
describe('indexed page fast path', () => {
  const turn = (base: number, n: number): SessionEvent[] => [
    ...
    { type: 'assistant/message', seq: SessionSeq(base + 2), time: base + 2, surfaceOp: 'append', data: { turn: n, step: 1, stream: [], message: { role: 'assistant', id: MessageId(`a${n}`), content: [{ type: 'text', text: `a${n}` }], source: { kind: 'model', provider: 'p', model: 'm' } } } },
  ...
  const mount = async (cut: number | undefined) => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    const sessionId = sid('session-indexed')
    const meta = header(sessionId, 1000)
    const events = [...turn(0, 1), ...turn(4, 2), ...turn(8, 3), ...turn(12, 4), ...turn(16, 5)]
  ...
  it('sizes the page from the indexed cut in one suffix read, never borrowing', async () => {
    const { ctx, sessionId, borrow, readFrom, messageCut, remote } = await mount(12)
    ...
    expect(messageCut).toHaveBeenCalledWith(sessionId, 2, 20, expect.anything())
    await ctx.fiber.dispose()
  })
  ...
  it('retries the indexed read once at the deep margin when a replacement widens the cut', async () => {
    ...
    expect(readFrom).toHaveBeenNthCalledWith(1, sessionId, cut - 128, base.length + 1, expect.anything())
    expect(readFrom).toHaveBeenNthCalledWith(2, sessionId, Math.max(0, cut - 4096), base.length + 1, expect.anything())
    expect(borrow).not.toHaveBeenCalled()
    await ctx.fiber.dispose()
  })
