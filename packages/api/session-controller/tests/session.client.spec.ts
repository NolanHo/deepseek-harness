...
it('publishes live chunk frames at frame cadence, not once per chunk', async ({ mock, start }) => {
  ...
  const session = await sessionBench(mock, start, SID)
  mock.stream(FOLLOW, followScript(history(plainTurn(SessionSeq(0), 0, '早', '安'))))
  await session.open()
  let notified = 0
...
describe('paging', () => {
  it('prepends an older page and keeps seq continuity', async ({ mock, start }) => {
    ...
    expect(mock.log.requests(PAGE)).toEqual([
      { address: ADDRESS, throughSeq: 11, beforeSeq: 6, maxMessages: 500, turnWindow: { minMessages: 8, minTurns: 2 } },
    ])
...
describe('prompt and cancel errors', () => {
  ...
  it('routes an addressed child through non-activating history, continuation prompt, and interrupt only', async ({ mock, start }) => {
    ...
    expect(mock.log.requests(FOLLOW)).toEqual([
      { address: { kind: 'subagent', ...CHILD }, assistantStream: true, maxMessages: 500, turnWindow: { minMessages: 8, minTurns: 2 } },
    ])
    expect(mock.log.requests(PAGE)).toEqual([])
  ...
  it('sends a one-shot address to the Host under the continuable marker', async ({ mock, start }) => {
    ...
    expect(mock.log.calls('subagents/interruptByParent').map(call => call.args)).toEqual([[SID, PARENT, 'continuable']])
    expect(mock.log.requests(FOLLOW)).toEqual([
      { address: { kind: 'subagent', ...CHILD, mode: 'one-shot' }, assistantStream: true, maxMessages: 500, turnWindow: { minMessages: 8, minTurns: 2 } },
    ])
    expect(mock.log.requests(PAGE)).toEqual([])
    expect(mock.log.requests('session/cancel')).toEqual([])
  })
...
describe('prompt rewrite options', () => {
  it('puts rewriteFrom on the wire and rebuilds the window before resolving when the Host reports rewrote', async ({ mock, start }) => {
    const session = await sessionBench(mock, start, SID)
    mock.stream(FOLLOW, followScript(history(plainTurn(SessionSeq(0), 0, '旧问', '旧答'), true)))
    await session.open()
    mock.remote.session.prompt.mockResolvedValue(ok({ accepted: true as const, rewrote: true }))
    mock.stream(FOLLOW, followScript(history([
  ...
  it('fails visibly when an armed rewrite is admitted without rewrote (not-yet-restarted Host)', async ({ mock, start }) => {
    const session = await sessionBench(mock, start, SID)
    mock.stream(FOLLOW, followScript(history(plainTurn(SessionSeq(0), 0, '旧问', '旧答'))))
    await session.open()
  ...
  it('ignores rewrite options on a subagent-addressed continuation', async ({ mock, start }) => {
    const session = await sessionBench(mock, start, SID, {
    ...
    expect(prompted).toEqual({ ok: true, value: { accepted: true } })
    expect(mock.log.requests('subagents/prompt')).toMatchObject([{ content: [{ type: 'text', text: '继续' }] }])
    expect(mock.log.requests('session/prompt')).toEqual([])
    expect(mock.log.requests(FOLLOW)).toHaveLength(1)
  })
})
