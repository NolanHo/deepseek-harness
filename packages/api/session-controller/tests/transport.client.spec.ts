...
describe('Session Client stream adapters', () => {
  ...
  it('binds an event journal to one address and publishes replace, append, and prepend changes', async () => {
    ...
    await stream.open({ maxMessages: 8 })
    await vi.waitFor(() => { expect(changes).toHaveLength(2) })
    await stream.prepend({ beforeSeq: 2, maxMessages: 8 })

    expect(remote.followRequests).toEqual([{ address: ADDRESS, maxMessages: 8 }])
    expect(remote.pageRequests).toEqual([
      { address: ADDRESS, throughSeq: 4, beforeSeq: 2, maxMessages: 8 },
    ])
