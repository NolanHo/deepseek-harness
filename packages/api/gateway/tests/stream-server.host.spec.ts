...
describe('Remote stream mux server carrier lifecycle', () => {
  ...
  it('negotiates permessage-deflate and carries a page-sized item through it', async () => {
    ...
    } as unknown as RemoteStreamOpener, 30_000, 262_144, undefined, true)
...
describe('Remote stream mux diagnostics', () => {
  ...
  it('reports the close code, reason, lifetime, and logical stream count of a peer close', async () => {
    const recorded = recordDiagnostics(3_000)
    const entry = await startDiagnosticMux(fastItem, 30_000, recorded.options)
    const client = await connect(entry.url)
  ...
  it('reports a first item slower than the configured threshold with its endpoint', async () => {
    ...
    const entry = await startDiagnosticMux(delayedItemOpener(50), 30_000, recorded.options)
    const client = await connect(entry.url)
    const messages = vi.fn()
    client.on('message', messages)
  ...
  it('emits nothing for a healthy carrier and reports only the close line', async () => {
    ...
    const entry = await startDiagnosticMux(fastItem, 20, recorded.options)
    const client = await connect(entry.url)
    const messages = vi.fn()
    client.on('message', messages)
...
/**
 * Start one mux carrying the fork's diagnostic sink, with upstream's uplink
 * inbox bound and a fixture Peer.
 * @param open - stream opener under test.
 * @param heartbeatIntervalMs - Ping interval in milliseconds.
 * @param diagnostics - sink and slow threshold for the carrier lines.
 * @param perMessageDeflate - negotiate RFC 7692 compression.
 * @returns the running mux.
 */
function startDiagnosticMux(
  open: RemoteStreamOpener,
  heartbeatIntervalMs: number,
  diagnostics: RemoteStreamDiagnostics,
  perMessageDeflate = false,
): Promise<RunningMux> {
  return startMux(open, heartbeatIntervalMs, 262_144, undefined, perMessageDeflate, diagnostics)
}
...
function delayedItemOpener(delayMs: number): RemoteStreamOpener {
  return async (_endpoint, _payload, _uplink, _peer, _control) => {
...
async function startMux(
  ...
  const mux = new RemoteStreamMuxServer(
    open, mapFailure, heartbeatIntervalMs, streamInboxBytes, perMessageDeflate, diagnostics,
  )
