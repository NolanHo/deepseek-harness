...
- Transport diagnostics write to the process's stderr because this tree registers no console exporter and cordis's built-in exporter only buffers. The mux takes its diagnostic sink as a constructor option, so a composition that does serve an exporter can pass that logger instead and route the same lines through it.
