...
export const ToolRow = memo(function ToolRow({
  ...
  // A caller-supplied body is content of its own: it expands the row even
  // while the call is still preparing.
  const hasBodyContent = bodyContent != null
  const expandable = hasBodyContent
    || (state !== 'preparing' && (inputRaw !== null || outputText !== null || card !== null))
  const open = expanded && expandable
  const bodyText = useMemo(
    () => open && !hasBodyContent && card === null && inputRaw !== null ? formatToolBody(variant, inputRaw) : null,
    [card, hasBodyContent, inputRaw, open, variant],
  )
  ...
  const expandedContent = useMemo(() => open ? (
    ...
    variant, bodyText, hasBodyContent, bodyContent, inputBody, outputText, state,
  ])
  return (
