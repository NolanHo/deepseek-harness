...
The node half snapshots each client bundle and available source map before publication. It groups resources into `/plugins/??...&rev=...` combo URLs, with one bootstrap combo for the modules row, one or more application combos for the other rows, and — for packages named by the `defer` config — deferred combos the shell fetches only after first mount; each phase is partitioned before a URL exceeds 3 KiB. Deferred rows must not be stage-one (`immediately`) and no pre-mount row may request them through `external` (both contradictions fail composition); a stale name in the list warns once and is ignored. Every combo map is Indexed Source Map v3 and uses an authored section when available or an identity section for the packaged bundle. Initial per-plugin revisions use process nonces, so startup does not hash every plugin; HMR hashes only an artifact reported as changed. Advertised responses are immutable, and an unknown combination or revision returns 404.
