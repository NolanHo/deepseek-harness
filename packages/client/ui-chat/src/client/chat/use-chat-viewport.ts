...
import { restoreAnchorOnReflow, type ReflowAnchor } from './fork/scroll-anchor.ts'
...
export class ChatViewport {
  ...
  // Fork patch (FORK_SURFACE.md): the reader row re-asserted after flow-height
  // changes that no prepend compensates (fold collapse, image loads, disclosures).
  ...
  /**
   * Hold one reader row for flow-height changes that no prepend compensates.
   *
   * Fork patch (FORK_SURFACE.md): the paging anchor is retained only across
   * history work, so a fold, image load, or disclosure above the reading line
   * expands the flow with no compensation.
   * @param position - semantic reader position to hold; null releases the hold.
   */
  armReflow(position: ChatScrollPosition | null): void {
    this.reflow.current = position === null ? null : { key: position.anchorKey, top: position.anchorTop }
  }

  /**
   * Re-assert the held reader row after a flow-height change and report the
   * re-captured position for the caller's scroll memory.
   * @returns the held reader position after compensation, or null when none is held.
   */
  restoreReflow(): ChatScrollPosition | null {
    const held = this.reflow.current
    const elements = this.elements
    if (held === null || elements === null) return null
    const next = restoreAnchorOnReflow(elements.list, elements.scroller, held, this.reflow, this.reflowLedger)
    if (next === null) return null
    return { anchorKey: next.key, anchorTop: next.top, scrollTop: elements.scroller.scrollTop }
  }
