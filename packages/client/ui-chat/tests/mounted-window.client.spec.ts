...
import {
  ...
  type HeadScroll, type MountedWindowInput,
...
describe('mounted window hook', () => {
  ...
  it('keeps the saved reader row mounted when a scroll steps the head', () => {
    ...
    // The saved row is the row the window must keep: the step is the full reveal
    // clamped only by that row, never by a stale head row.
    act(() => { expect(result.current.revealAtHead(headScroll(), false)).toBe(true) })
    // The step keeps both rows: the head the reader reached (k125) and the saved
    // row (k150) the window still holds, so the slice starts one row lower.
    expect(result.current.headKey).toBe('k101')
