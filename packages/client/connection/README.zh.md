...
同一份列表经 boot wire 到达浏览器：node 半侧把它的 `trustedHosts` 配置发布为 `window.__DSH_BOOT__.trustedAuthorities`（从不包含回环本身），客户端把该列表中的页面权威视为第一方——即 settings 平面选择 `'host'`/`'memory'` 持久化所依据的 `ctx.connection.isServingAuthority` 状态。经回环改写代理提供的部署（前端在请求到达后端前把 Host/Origin 改写为 127.0.0.1）因此能向其浏览器提供 settings 平面；上述栅栏仍是服务端门禁，`trustedHosts` 仍是可达性策略而非认证。
