...
export interface ConnectionHandle {
  ...
  /**
   * Fork-compatible alias of {@link ConnectionHandle.isLoopback}: whether this
   * deployment serves the current page authority (loopback, transport-owned,
   * or listed in the host-published boot wire). Consumers outside this package
   * still read `isLoopback`; the fork's settings plane reads `isServingAuthority`.
   */
...
export function apply(ctx: Context): void {
  ...
  const servingAuthority = transport?.ownsHost === true
  ...
  const handle: ConnectionHandle = {
    ...
    isServingAuthority: servingAuthority,
    generation: {
