...
type ChatActions = {
  ...
  toggleTurnFold: (draft: ChatStoreState, turn: number) => void
}
...
export function createChatStore(): EngineStoreHandle<ChatStoreState, ChatActions> {
  return defineStore({
    ...
    init: (): ChatStoreState => ({ selection: null, draft: '', view: null, inspect: null, expandedTurns: [] }),
    ...
    actions: {
      ...
      toggleTurnFold: (d, turn: number) => {
        const previous = d.expandedTurns ?? []
        d.expandedTurns = previous.includes(turn)
          ? previous.filter(candidate => candidate !== turn)
          : [...previous, turn]
      },
    },
  })
