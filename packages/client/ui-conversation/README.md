...
- **No retry for a continuable child's parent catalog** — the `subagents.list` read is the only writer of `parentAvailable`; a failed or stalled read leaves the child unknown until a connection reset or the parent's catalog menu opens, and only the bounded settle hide covers the gap.
