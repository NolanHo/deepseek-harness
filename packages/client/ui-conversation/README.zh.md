...
- **continuable 子会话的父目录没有重试**——`parentAvailable` 的唯一写入方是 `subagents.list` 读取；读取失败或挂起时，子会话在下次连接重置或父会话目录菜单打开前一直处于未知，这段空档只由有上限的 settle 隐藏兜住。
