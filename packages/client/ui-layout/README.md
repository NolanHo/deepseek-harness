...
Mobile viewports (below the 768px `MOBILE_VIEWPORT` breakpoint) drop the three-column grid for a single center track. The sidebar renders inside an always-mounted fixed drawer that slides in over a scrim; `toggleSidebar` flips the drawer flag, and the drawer closes on scrim tap, Escape, or a session change. The details column becomes a fixed right sheet (`min(92vw, 420px)`) whose open preference drives the slide, with a frame-owned close button. `ctx.layout.closeDrawer()` is the cross-plugin face app views call when they take over the center column; Escape closes the sheet before the drawer when both are open, and drag handles plus the concession solver are skipped in this regime.
