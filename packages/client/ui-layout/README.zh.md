...
移动端 viewport（低于 768px 的 `MOBILE_VIEWPORT` 断点）放弃三栏网格，只保留单列会话栏。侧栏渲染在始终挂载的固定抽屉内，通过遮罩滑入；`toggleSidebar` 翻转抽屉开关，抽屉在点击遮罩、按下 Escape 或会话变更时关闭。详情栏变为固定右侧面板（`min(92vw, 420px)`），由打开偏好驱动滑入，并带一个由外壳持有的关闭按钮。`ctx.layout.closeDrawer()` 是应用视图接管会话栏时调用的跨插件接口；两层同时打开时 Escape 先关闭详情面板再关抽屉，该形态下隐藏拖动手柄并跳过让步求解器。
