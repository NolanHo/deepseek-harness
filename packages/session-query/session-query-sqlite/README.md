...
| `maxPersistedObservedEvents` | `5000` | Largest number of persisted-Session events one search request may cold-read to index changed or unindexed stored logs; a request that reaches it stops cold-reading and leaves the remainder to later requests |
...
Reconciliation runs once per caller request. The provider memoizes the pass it completed against the request's `exec.reconciliationBudget`, so every page of a cursor sequence observes one corpus and cannot re-list, re-read, or re-index the stored logs between pages. That pass cold-reads the stored log of every persisted Session the index does not hold at its current revision, charges each read against the same budget, and re-indexes the Session's documents; when the request reaches the provider's `maxPersistedObservedEvents`, it stops cold-reading, commits the Sessions it read, and completes with the corpus it has. The remaining Sessions are indexed by later requests, so a large pending set converges across searches instead of refusing every search or reading the whole store in one request, and the stability retry cannot re-read past the bound.
...
Persisted FTS rows live in a dedicated derived database and survive restarts; live sessions use connection-local TEMP tables that shadow the durable base for the same session and reveal it again when the live owner detaches. Both tables retain the exact inherited cut in numeric `seed_length`; reconstructed headers expose only `isSeeded`, while the cut participates in live fingerprints and persisted source revisions. Each search runs one serialized observation: list persistence snapshots, compare per-session revisions with the indexed rows, read only new or changed logs through a read handle (balancing an interrupted final turn in memory, never writing back), extract semantic documents, and commit the reconciliation in one transaction before running the query. Repeated queries and unchanged reopens read nothing; switching stores or observing new, changed, deleted, or externally repaired sources reconciles on the next stable observation. Source or transaction failure commits nothing and the next search retries. One reconciliation serves one caller request: its pages share the provider's memo keyed by the request's reconciliation budget, and the persisted cold reads that a catch-up pays are charged against that request's `maxPersistedObservedEvents`.

A Session's documents are deleted through the rowid range recorded in `persisted_doc_ranges` (and its TEMP counterpart `live_doc_ranges`) rather than by `session_id`: `session_id` is `UNINDEXED`, so a delete by it scans the whole full-text table, which made re-indexing one changed Session cost the entire index. Rows written in one synchronous pass are contiguous, so the recorded range is exactly that Session's documents, and a rowid equality is a docid lookup instead of a scan.
