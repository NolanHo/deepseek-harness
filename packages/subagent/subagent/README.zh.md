...
列举一个本进程尚未加载的子级，需要读取该子级完整的已存会话日志一次。两个 `config` 字段为单次列举的这类读取设界：`coldReadConcurrency` 限制同时进行的读取数（默认 `4`），`coldReadBudget` 限制单次列举最多发起多少次读取（默认 `64`）。超出预算的候选会给出可重试的 `unavailable` 诊断，由后续列举接手；投影缓存会应答上一次列举已解析的子级，因此反复列举能在冷工作区中持续前进，而不是反复读取同一批头部候选。
