...
describe('SubagentRuntime.listChildren', () => {
  ...
  it('lists provider-established children from the parent catalog without a corpus or child read', async () => {
    ...
    // The rows are exactly the parent's catalog projection: one parent
    // observation, no corpus listing, and no child observation.
    expect(children).toEqual(
      ctx.sessionProjections.snapshot(parent.session, ['subagentCatalog']).values.subagentCatalog,
    )
    expect(listSessions).not.toHaveBeenCalled()
    expect(observeSession.mock.calls.map(([id]) => id)).toEqual([parent.id])
  })
...
/** The positioned descendant row one synthetic cold candidate of the corpus resolves to. */
...
async function setupColdCorpus(
  ...
  await ctx.plugin(SessionProjectionRegistry)
  await ctx.plugin(TestSessionQuery)
  // An omitted config object is the shipped-schema-defaults path; only an
  // explicit object carries validated bounds into the service.
  if (options.config === undefined) await ctx.plugin(SubagentRuntime)
  else await ctx.plugin(SubagentRuntime, options.config)
  return { ctx, observed: serveColdCorpus(ctx, headers, options.observe) }
}

// Fork patch (FORK_SURFACE.md): pins the descendant listing's per-listing
// cold-read bounds, the projection-cache rung that spends no budget, and the
// retryable `unavailable` degradation.
describe('SubagentRuntime.listDescendants cold-read bounds', () => {
  it('defaults both cold-read bounds and rejects a bound below one', async () => {
    const defaults = SubagentRuntime.Config({})
    expect(defaults.coldReadConcurrency).toBe(4)
    expect(defaults.coldReadBudget).toBe(64)
    // The volatile capacity fields resolve their own schema defaults beside the bounds.
    expect(defaults.maxActiveSubagents.get()).toBe(8)
    expect(defaults.maxDepth.get()).toBe(1)
    const configured = SubagentRuntime.Config({ coldReadBudget: 8 })
    expect(configured.coldReadConcurrency).toBe(4)
    expect(configured.coldReadBudget).toBe(8)
  ...
  it('reads only the leading cold candidates a listing budget allows', async () => {
    ...
    const entries = await ctx.subagents.listDescendants(COLD_CORPUS_PARENT)

    expect(observed).toEqual([headers[0]!.id, headers[1]!.id])
    expect(entries).toEqual([
      coldChildRow(headers[0]!),
      coldChildRow(headers[1]!),
      { kind: 'diagnostic', id: headers[2]!.id, reason: 'unavailable', parentId: COLD_CORPUS_PARENT, depth: 1 },
      { kind: 'diagnostic', id: headers[3]!.id, reason: 'unavailable', parentId: COLD_CORPUS_PARENT, depth: 1 },
      { kind: 'diagnostic', id: headers[4]!.id, reason: 'unavailable', parentId: COLD_CORPUS_PARENT, depth: 1 },
    ])
  })
  ...
  it('treats a budget of one as one read and defers the rest', async () => {
    ...
    expect(entries).toEqual([
      coldChildRow(headers[0]!),
      { kind: 'diagnostic', id: headers[1]!.id, reason: 'unavailable', parentId: COLD_CORPUS_PARENT, depth: 1 },
    ])
  })
  ...
  it('keeps concurrent cold observations at the configured concurrency', async () => {
    ...
    const listing = ctx.subagents.listDescendants(COLD_CORPUS_PARENT)
  ...
  it('advances past cache-served children on the next listing', async () => {
    ...
    const first = await ctx.subagents.listDescendants(COLD_CORPUS_PARENT)
    expect(observed).toEqual([headers[0]!.id, headers[1]!.id])
    ...
    await expect(ctx.subagents.listDescendants(COLD_CORPUS_PARENT))
      .resolves.toEqual(headers.map(coldChildRow))
    expect(observed).toEqual(headers.map(header => header.id))
  })

  it('applies the shipped cold-read bounds to a service mounted without config', async () => {
    const headers = [1, 2, 3, 4, 5, 6].map(rank => coldHeader(`cold-unconfigured-${rank}`, rank))
    const gate = Promise.withResolvers<undefined>()
    let inFlight = 0
    let peak = 0
    // No config object reaches the loader, so the shipped schema bounds this
    // listing: four concurrent reads inside the default budget of 64.
    const { ctx, observed } = await setupColdCorpus(headers, {
      observe: (header) => {
        inFlight += 1
        peak = Math.max(peak, inFlight)
