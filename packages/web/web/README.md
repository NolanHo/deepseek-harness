...
Each call resolves its provider at execution time, and registration or load order never matters. A configured provider id wins when it is registered and usable; without a configured id, the service runs the single usable provider or fails clearly. A `search()` call may additionally pass `options.provider`, which overrides the configured default for that call alone (the `web_search` tool exposes it as its `backend` argument):

| Situation | Outcome |
|---|---|
| per-request id registered and usable | runs that provider for the call |
