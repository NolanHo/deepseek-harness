...
| 工具 | 参数 | 行为 |
|---|---|---|
| `web_search` | `queries`（必填 string[]）、`backend`（string，可选） | 用于发现信息。返回可选答案与来源 URL。它会并发执行 1 至 `searchMaxQueries` 个不同搜索，按轮询顺序合并来源，再应用组合后的 `searchMaxResults` 上限。单元素数组执行一次搜索。完全相同的查询只执行一次。任何搜索失败都会中止批次中的其余搜索；批次结算完毕后调用才返回错误。`backend` 仅为该次调用指定一个已注册提供方（省略 = 配置的默认提供方）；未知后端会失败并列出可用项。两个上限都不面向模型。 |
...
schema 校验会在执行前拒绝缺失或非数组的 `queries` 字段以及非字符串数组元素。值错误精确地变为 `Error: queries must contain at least one query`、配置上限为 1 时的 `Error: queries must contain at most 1 query`、上限更大时的 `Error: queries must contain at most <count> queries`、`Error: each query must be a non-empty string`、`Error: backend must be a non-empty string when supplied` 或 `Error: url must be a non-empty string`。
