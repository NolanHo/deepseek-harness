...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'abstract messageCut(id: SessionId, maxMessages: number, beforeSeq?: number, signal?: AbortSignal): Promise<number | undefined>',
        description: 'The seq of the `maxMessages`-th append-origin user message counting back from the log tail (or from `beforeSeq` when given), or undefined when the backend cannot answer (sequential media) or the log holds no matching message. The indexed seek that sizes one history page exactly; the same population readFrom-based pagination counts.',
        parameters: [{ name: 'id', description: 'the persisted session to index.' }, { name: 'maxMessages', description: 'message rank to locate (1 = the last user message).' }, { name: 'beforeSeq', description: 'optional exclusive upper bound (loadOlder pages).' }, { name: 'signal', description: 'optional cancellation for backend read work.' }],
