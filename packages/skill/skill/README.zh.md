...
- **scope 目录限制。** scope 化的上下文用 `ctx.skills.restrict({ allow, deny })` 收窄其目录视图；保留名单与排除名单互斥，被限制的名字在该 scope 链的 `snapshot`/`list`/`get` 中读作不存在。该 scope 自身的注册不受限，与 `tools.restrict` 对称。
...
注册表没有 TTL：只有提供方调用其注册作用域内的 `invalidate()`、发生运行时注册或释放、或 scope 限制的落上与解除时，才会清除已完成的目录。每次失效都会递增 revision、清空缓存，并发出不带过滤条件的 `skills/change` 事件；消费方用各自的查找选项重新获取。`invalidate()` 仅当接收它的那条精确注册仍处于活动状态时才生效，因此延迟回调无法干扰同名替代提供方。
