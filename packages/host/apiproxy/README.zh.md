...
`session.history` 会读取已附加 Session 的内存状态，或通过分页持久化读取冷日志（`readFrom` 后缀窗口：可用时按后端消息索引切点定窗（SQLite 一次索引扫描即答），否则回退到保守估计与按密度重估的加宽循环——SQLite 这类可定位后端只读窗口，非 seek 后端在协调器内部降级），而不会恢复或发布 agent。连续 delta-chunk 流用存储编解码器折叠成单个 `{ packed }` 线上条目，客户端再展开回精确的事件；fetch 载体对值得压缩的 JSON 响应按客户端声明的最强编码（zstd，其次 gzip）压缩。页切点落在用户消息上：一页 = 整数个回合——切点在用户提问处，该回合完整的工具/助手内容随页携带，绝不切在回合中间或回答中间；没有任何用户消息的日志回退到 assistant 消息边界。`maxMessages` 统计以追加方式进入 surface 的 `user/message` 事件，因此仅供模型使用的替换副本不占用配额。每一页仍是一段连续的原始事件区间，从而让压缩（compaction）的仅日志 `compaction/summary` 记录与引用它的替换留在同一页。修复歧义的尾部（未闭合的最后回合，或无法证明已闭合的窗口）回退到全量 inspection 以合成中断回合的闭合事件；尾页的 `projections` 基线来自投影缓存的冷读阶梯，fail-soft。
