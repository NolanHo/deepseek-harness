...
请求从 dist 根目录（包含 `distIndex` 的目录）提供。dist 根目录与配置的 index 路径以 HTTP 200 渲染 `index.html`；任何其他已有文件按自身 MIME 类型直接提供，未知扩展名按 `application/octet-stream` 提供。位于内容哈希输出目录（`assets/`、`preview/`——其中每个产物名都带内容哈希）内的文件额外回答 `cache-control: public, max-age=31536000, immutable`；index 响应与稳定命名的根文件不带缓存指令。该响应头是本 fork 的部署差异，登记在 [FORK_SURFACE.md](../../../FORK_SURFACE.md)。解析到根目录之外的路径以 403 拒绝，因此精心构造的路径无法读取 dist 之上的文件。dist 根目录内缺失或非文件的 target——文件缺失、目录或配置的 index 缺失——返回空 404。没有匹配具名路由的非 GET／HEAD 请求回答 405。每个成功的 index 响应都经 webserver 的 `renderIndex` 渲染，因此启动 manifest 在 `/` 与配置的 index 路径上到达页面。
