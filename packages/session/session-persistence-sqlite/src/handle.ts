/**
 * The SQLite provider's session storage runtime: its concrete write/read
 * handle with a per-handle mutation chain and a routed live write-behind
 * buffer, plus the in-process bookkeeping that enforces one active writer per
 * session id. Deliberately provider-local: the persistence seam exposes only
 * the service and handle contracts.
 * @module @deepseek-ai/dsh-session-persistence-sqlite/handle
 */
...
export interface SqliteHandleStorage {
  /** Append a contiguous batch; `isMaterialized` selects create-vs-extend publication. */
  persistBatch(
...
export interface SqliteHandleState {
  /** The stored next-seq (the logical end this handle knows). */
  cursor: number
  /** Whether the session has a durable row yet. */
  materialized: boolean
  /** Physical deletion base for a torn tail, consumed by the first new append. */
...
export class SqliteSessionHandle implements SessionHandle {
  ...
  constructor(
    ...
    private readonly state: SqliteHandleState,
  ...
  /**
   * Durability barrier; materializes the row when nothing has been appended
   * yet, so an explicitly flushed empty session survives this process.
   ...
   * unwinds first; a drain failure still frees the in-process claim, then
   * rejects.
  ...
  close(): Promise<void> {
    return this.closing ??= (async () => {
      ...
      await this.chain
      // Free the in-process claim no matter how the drain fared: a skipped
      // release would wedge the id behind a stale ownership claim.
  ...
  private async persistContiguous(batch: readonly SessionEvent[]): Promise<void> {
    ...
    // Truncate any pending torn tail first, clearing the marker only once the
    // repair lands so a failed step retries on the next mutation.
    if (this.state.tornTruncateTo !== undefined) {
      await this.storage.truncateTornTail(this.header, this.state.inheritedEventCount, this.state.tornTruncateTo)
...
/**
 * The SQLite backend's in-process bookkeeping: the single active writer per
...
export class SqliteBackendTracker {
   ...
   * collide across processes, and no durable row exists for another process
   * to open.
   ...
   * close drains the routed buffer — and aggregates failures. Registrations
   * are effects of the current fiber.
