...
每个连接都会禁用 SQLite trusted schema 与内存映射 I/O、验证所请求的 journal mode，并固定 `synchronous=FULL`，保证成功返回的追加在操作系统崩溃或断电后依然持久。配置 `cacheSizeKib` 时，该连接使用所请求的页缓存，而非 SQLite 默认建议的 2,000 KiB（`-2000`，约 1.95 MiB）；省略该字段，或把 `cordis.yml` 中的值留空（以 null 传入），则不执行任何 pragma、保持该默认值，而 `0` 会应用 `-0`，即一个零页建议，SQLite 会将其下限钳制为 10 页。在 POSIX 上，数据库父目录和文件必须属于当前用户，父目录不得允许组或其他用户写入，文件也不得授予任何组或其他用户权限；Windows 还会拒绝符号链接和非普通文件，ACL 限制则由部署方负责。路径与所有权失败会拒绝插件初始化；Node 的 SQLite 驱动在首次持久化操作时才延迟加载。普通 `create` 会保持惰性直到首次 append，而 `ensureMaterialized` 会写入一条没有事件行的会话元数据记录。
...
| [`src/child-catalog.ts`](src/child-catalog.ts) | 历史恢复：从 sessions 与 events 行读取父会话的直接子会话目录证据 |
...
完整读取先反向定位最后一个有效 `turn/end`，再按正向顺序把每个物理行解码为其逻辑事件，并拒绝已提交前缀中的缺口或格式错误行。格式错误的最后一行被视为撕裂尾部：执行恢复的加载可以在写锁下删除它，并用合成闭合事件关闭日志。后缀读取（`readFrom`）只检查可能包含目标序列的物理跨度，因此永远不会解析无关的更早行；带排他上界的读取只选择首个逻辑 seq 位于该上界之前的行，并在自己的读事务内跑一次纯索引的 `MAX(seq)` 以报告它观测到的已存末端（空日志为 `-1`）——上界使该读取够不到这个末端——而扫描若未到达该上界就退回无界重读，因此严格位于上界之下的损坏保持完整读取给出的分类，位于上界及其之上的损坏则始终不被读取。

存储版本低于当前 Session 格式的行经已发布的格式 catalog 恢复，其 V3→V4 边要求父会话给出完整的直接子会话证据，否则拒绝历史正文。本提供方不从子日志、而是从自己的表收集该证据：`sessions.parent_session` 配合 `origin = 'subagent'` 选出直接子会话，每个子会话贡献其存储的会话键、创建时间与 `COALESCE(seed_length, 0)` 切点，`events` 回答该子会话自身的 `subagent/descriptor` 数量，并在恰好一条时回答用 schema 字典解码的首个载荷。一个连接对某个父会话的子会话只收集一次，并把结果与解码日志缓存并列保留；每次已提交的变更与 `close()` 都会丢弃它。

存储身份无法被本提供方校验的子会话不携带父目录可记录的身份，因此只跳过该子会话；自身 descriptor 不可读、无法解码，或带有目录拒绝的字段的子会话，保留仅含身份的未知模式归属。因此单个不可用子会话不会拒绝其父会话的恢复。与父会话自身已记录的目录项真正冲突的证据仍然拒绝恢复，被拒绝的迁移不发布任何内容。
...
`decodedLogCacheBytes` 为单个连接保留整份已解码日志。只有当同一次调用读到的会话行携带该条目被读入时的 revision 时，读取才会复用该条目，因此任何连接或进程的写入都会未命中，而不会返回数据库已不再持有的事件。复用判定就在这个读事务内、且在查询事件行之前做出，因此命中只读一行会话行、不读任何事件行。追加、发布、修复、truncate 以及 header 物化还会额外删除该会话的条目，`close()` 则删除全部条目。上限按每条日志数据列的解码后 JSON 文本计费——压缩列按解压后的文本——并持续淘汰最久未使用的条目，直到保留总量落入上限；超过整体上限的日志永不保留。
...
- **解码日志缓存按解码文本计费**——`decodedLogCacheBytes` 按各行解出的 JSON 文本的 UTF-8 字节数为每条保留日志计费，而实际保留的是由该文本解析并冻结的事件图；行内没有文本的会话（如仅有 header 的空会话）不占字节（因此字节上限并不约束条目数）。请把该上限当作留有冗余的缓存大小，而不是内存预算。
- **采集到的子会话证据与保留窗口一致**——历史父会话的子会话每个连接只采集一次，并随其解码日志在同一批变更中被丢弃，因此其他进程在本连接最后一次变更之后写入的子会话行，要等到该连接再次变更或重新打开时才被采集。
- **缓存条目存活期间会掩盖带外物理损坏**——revision 校验能覆盖本提供方的全部写入，但在它之外直接执行的 SQL（外部工具、共用同一文件的第二个实例）可以在不推动 revision 的情况下改动行，此时该会话的保留日志会继续作答直到条目被淘汰，收缩保护（shrink guard）的损坏判定也会被一并掩盖。把备份恢复到仍在运行的 store 之上属于同一类就地损坏：恢复前先停实例，让进程打开恢复后的文件，而不是继续持有被替换掉的那一个。
