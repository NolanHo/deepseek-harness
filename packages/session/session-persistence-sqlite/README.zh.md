...
- **缓存条目存活期间会掩盖带外物理损坏**——revision 校验能覆盖本提供方的全部写入，但在它之外直接执行的 SQL（外部工具、共用同一文件的第二个实例）可以在不推动 revision 的情况下改动行，此时该会话的保留日志会继续作答直到条目被淘汰，收缩保护（shrink guard）的损坏判定也会被一并掩盖。
