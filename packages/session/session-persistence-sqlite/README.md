...
| [`src/child-catalog.ts`](src/child-catalog.ts) | Historical restore: the parent's direct-child catalog evidence read from the sessions and events rows |
...
A full read locates the last valid `turn/end` in a reverse pass, then decodes each physical row into its logical events in forward order, rejecting gaps or malformed rows in the committed prefix. A malformed final row is treated as a torn tail: a mutating load may delete it under the write lock and close the log with synthetic closers. Suffix reads (`readFrom`) examine only the physical span that may contain the requested sequence, so they never parse unrelated earlier rows; a read that carries an exclusive upper bound selects only rows whose first logical seq precedes it, runs one index-only `MAX(seq)` inside its own read transaction to report the stored end it observed (`-1` for an empty log) — a bound keeps the read away from that end — and re-runs the read unbounded when a scan stops short of the bound, so damage strictly below the bound keeps the classification a full read gives it while damage at or above the bound stays unread.

A stored version below the current Session format restores through the released format catalog, whose V3→V4 edge refuses a historical body unless the parent supplies its complete direct-child evidence. This provider collects that evidence from its own tables instead of reading child logs: `sessions.parent_session` with `origin = 'subagent'` selects the direct children, each child contributes its stored session key, creation time, and `COALESCE(seed_length, 0)` cut, and `events` answers the child's own `subagent/descriptor` count and, for exactly one, its first payload decoded with the schema dictionary. One connection collects a parent's children once and retains the result beside the decoded-log cache; every committed mutation and `close()` drop it.

A child whose stored identity this provider cannot validate carries no identity a parent catalog may record, so only that child is skipped; a child whose own descriptor is unreadable, undecodable, or carries fields the catalog refuses keeps identity-only unknown-mode membership. One unusable child therefore cannot refuse its parent's restore. Evidence that genuinely conflicts with the parent's own recorded catalog entries still refuses the restore, and a refused migration publishes nothing.
...
`decodedLogCacheBytes` retains whole decoded logs for one connection. A read reuses a retained log only when the session row it reads in that same call carries the revision the entry was read at, so a write from any connection or process misses instead of returning events the database no longer holds. The reuse decision is made inside that read transaction and before the event rows are queried, so a hit reads one session row and no event rows. Append, publish, repair, truncate, and header materialization additionally drop the session's entry, and `close()` drops every entry. The ceiling charges each log the decoded JSON text of its data columns — a compressed column's decompressed text — and evicts least-recently-used entries until the retained total fits; a log larger than the whole ceiling is never retained.
...
- **Collected child evidence shares the retention window** — a historical parent's children are collected once per connection and dropped by the same mutations that drop its decoded log, so a child row another process writes after this connection's last mutation is not collected until that connection mutates again or reopens.
