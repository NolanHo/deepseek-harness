...
describe('SQLite compression', () => {
  ...
  it.each(['text-chunks', 'reasoning-chunks', 'tool-call-chunks'])(
    ...
    (type) => {
      ...
      expect(physical.ignorable).toBeNull()
      expect(decodeRow(physical)).toEqual([logical])
    },
  )

  it('stores the ignorable envelope marker as its column value', () => {
    const base: SessionEvent<'turn/start'> = {
      type: 'turn/start', seq: SessionSeq(0), time: 1, data: { turn: 1 },
    }
    const flagged: SessionEvent<'turn/start'> = { ...base, ignorable: true }
    expect(row(flagged).ignorable).toBe(1)
    expect(row(base).ignorable).toBeNull()
    expect(decodeRow(row(flagged))).toEqual([flagged])
    expect(decodeRow(row(base))).toEqual([base])
  })
