...
| `messageCut(id, maxMessages, beforeSeq?, signal?): Promise<number \| undefined>` | The seq of the `maxMessages`-th append-origin user message counting back from the log tail (or from `beforeSeq`) — the indexed seek that sizes one history page exactly — or undefined when the backend cannot answer or no matching message exists. SQLite answers in one indexed scan; sequential media return undefined so callers fall back to their own estimate. |
...
| `userMessageCut?(id, maxMessages, beforeSeq?, signal?)` | Optional indexed message-rank seek behind the service's `messageCut`: the seq of the Nth append-origin `user/message` counting back from the tail (or `beforeSeq`), or undefined when the log holds no matching message. SQLite implements it (`LIMIT` over `type`/`surface_op`); backends that omit it make `messageCut` answer undefined. |
