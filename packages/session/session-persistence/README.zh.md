...
| 文件 | 职责 |
|---|---|
| `messageCut(id, maxMessages, beforeSeq?, signal?): Promise<number \| undefined>` | 从日志尾部（或从 `beforeSeq`）往回数第 `maxMessages` 条追加来源用户消息的 seq——精确给历史页定窗的索引寻址——后端无法回答或无匹配消息时为 undefined。SQLite 一次索引扫描即答；顺序介质返回 undefined，调用方回退到自己的估计。 |
...
| `userMessageCut?(id, maxMessages, beforeSeq?, signal?)` | 服务 `messageCut` 背后的可选索引消息寻址钩子：从尾部（或 `beforeSeq`）往回数第 N 条追加来源 `user/message` 的 seq，日志无匹配消息时为 undefined。SQLite 实现（对 `type`/`surface_op` 的 `LIMIT`）；未实现的钩子使 `messageCut` 回答 undefined。 |
-----
