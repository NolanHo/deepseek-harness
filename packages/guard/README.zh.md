---
description: "循环卫生 guard 家族的包映射：建议性重复工具提醒、单次工具调用超时策略，以及按需启用的轮次结束续跑策略，供选择或组合 guard 的用户与维护者阅读。"
kind: "package-group"
---
...
`guard/` 组通过监视三种常见失败模式来保持 agent loop（智能体循环）高效。`repeat-tool-reminder` 会在模型重复相同的工具调用时提醒它，使其改变方法或结束任务。`timeout-policy` 为声明了限时的工具调用设置超时，返回清晰的错误而不是无限等待。`turn-continuation` 在当前轮次以配置的原因结束时开启新的一轮，默认 `max-tokens`，续跑次数按人类输入设限、且只由人类输入补充。前两者随 `dsh` base 组合默认启用；`turn-continuation` 为按需启用，由一行指定 `@deepseek-ai/dsh-turn-continuation` 的 profile patch 启用。
...
- [工具子系统参考](../../docs/subsystems/tools.zh.md)——两个工具调用 guard 所依据的工具调用流水线与决策。
- [生成配置目录](../../docs/config-catalog.zh.md#deepseek-aidsh-repeat-tool-reminder)——重复调用提醒的每个受支持字段。
- [生成配置目录](../../docs/config-catalog.zh.md#deepseek-aidsh-turn-continuation)——轮次结束续跑策略的每个受支持字段。
