...
**冷历史读取改为带加宽循环的分页后缀读取。** api proxy 的 `detachedHistoryRead` 先用 `persistence.list()` 解析会话头（缺失映射为 `session-not-found`），选择首个窗口（`fromSeq`）：tail 页锚定在投影缓存水印减去 `maxMessages × 4096` 事件处，loadOlder 页锚定在 `beforeSeq` 减去同一估计处——故意激进，因为 SQLite 单行读取便宜、延迟跳变只出现在跨越存储页时——循环：`readFrom(id, fromSeq)` → `paginate` → 若页切点可证明落在后缀内（`cut >= fromSeq`）且窗口至少含 `maxMessages` 条用户消息则直接服务；否则按后缀实测的每消息事件密度重估 `fromSeq`（翻倍余量，保留仅防无进展的减半兜底）再读。`fromSeq === 0` 构造上即精确，循环必然终止；最坏情况等于今天的全量读。非 seek 后端（JSONL）在协调器内部降级（`readFrom` 回退为全前缀 + 跳过），不低于今天的行为。
...
- **减半 `fromSeq`（初版实现）**：已替换——一步减半可能超调一个数量级（密集 agent 会话上 6.4k 事件窗口一跳 27.5 万，~800ms 浪费解码）；密度重估两次读取即收敛。
