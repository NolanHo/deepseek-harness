...
两条快速路径都从 `messageCut` 取切点，而该查询按存储物理 `seq` 列筛选，并且都通过 `readFrom` 读事件。只有当前格式行能让两者处于同一空间：`loadStoredFrom` 的历史分支会用格式目录恢复整段日志，而这次恢复会按恢复后的事件重排序号，因此物理切点在那里指不到任何东西。生产库绝大多数是历史行，于是跳过该检查的窗口计划每次尝试都要付一次切点查询加一次整段日志读取，然后再回落到观测路径、再读一遍日志——比没有这条快速路径还慢。因此门槛是 `readIndexedSuffix` 的第一件事：`SeekablePersistence.seekable(id)` 询问后端能否寻址有界 seq 窗口，`SqliteStore.seekable` 只凭会话行的格式版本作答（不读任何事件行），返回 false 时在 `messageCut` 与任何 `readFrom` 之前就退出。`loadStoredFrom` 的历史分支继续服务整段日志读取者，并在其契约中写明它无法服务索引分页切点。
...
门槛由存储层用例（迁移后的 v0 行把两个空间量出来：存储行为 `[0, 1, 2, 5, 6]`，恢复序号为 `[0..5]`；物理切点取不到任何事件；只有发布后该行才可寻址）与宿主用例（历史会话走观测路径且 `messageCut`/`readFrom` 调用数为零）以及一条探针先行的单元用例钉住。
