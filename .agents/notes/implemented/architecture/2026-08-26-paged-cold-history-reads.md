...
**Cold history reads are paged suffix reads with a widening loop.** `detachedHistoryRead` in the api proxy resolves the session's header through `persistence.list()` (mapping an absent session to `session-not-found`), picks a first window (`fromSeq`) anchored at the projection cache's stored watermark minus `maxMessages × 4096` events (tail pages) or at `beforeSeq` minus the same estimate (loadOlder pages) — deliberately aggressive, because one SQLite row is cheap and the latency jump only appears crossing storage pages — and loops: `readFrom(id, fromSeq)` → `paginate` → if the page cut provably lands inside the suffix (`cut >= fromSeq`) and the window holds at least `maxMessages` user messages, serve; otherwise re-estimate `fromSeq` from the suffix's observed events-per-message density (doubled as headroom, with a halving fallback that only guards non-progress) and re-read. `fromSeq === 0` is exact by construction, so the loop always terminates; the worst case is today's full read. A non-seek backend (JSONL) degrades inside the coordinator (`readFrom` falls back to full prefix + skip), never below today's behavior.
...
- **Widen by halving `fromSeq` (the first implementation)**: replaced — one halving step can overshoot by an order of magnitude (a 6.4k-event window jumping to 275k on dense agent sessions, ~800ms of wasted decode); density re-estimation converges in two reads.
