...
The collapsed Think row renders a reasoning character count instead of text, so its summary width is fixed per row and its content changes at most once per published frame; no horizontal scrolling, synchronous layout read, or smooth-scroll animation sits in the publication path. Chat body scrolling, history-prepend anchoring, and user-triggered `scrollIntoView` are untouched ([collapsed Think character count](../feature/2026-09-12-collapsed-think-character-count.md)).

`pnpm run test:web:stress` remains keyless, opt-in browser performance evidence. A test-owned model adapter emits 100,000 `reasoning-delta` chunks through the real Host, Gateway, WebSocket, Session reduction, and live Think row, and the live Think row's displayed character count reaching the adapter's emitted reasoning length proves that the events cross production session reduction and reach that row. Each 128-chunk batch waits for one browser timer turn before the next batch, so a 50-millisecond heartbeat and a pre-scheduled DOM event measure one bounded ingestion/render interval instead of accumulated socket backlog against a 250-millisecond budget. A terminal marker proves that all chunks reached the UI. `DSH_WEB_STRESS_HEADFUL=1` lets developers profile the same scenario in a visible browser with the Performance panel. The stress lane is evidence for manual performance diagnosis and fix acceptance, not a default CI gate or a substitute for deterministic scheduling unit tests.
...
The collapsed Think summary holds no horizontal position of its own: the row renders a character count whose width is fixed per row, so nothing in the summary depends on the accumulated text's layout. React still commits accumulated snapshots normally, and body scrolling or user interactions keep their immediacy.
