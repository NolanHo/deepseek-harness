# Agent Note: The busy Send button follows the busy-Enter setting
...
The Settings row is retitled to cover both inputs: "Send behavior while busy" / "繁忙时的发送行为", described as how the submit chord and the Send button deliver while the agent is running. The `busyEnter` field name, its `queue` default, and the Host schema are unchanged, so existing `settings.yaml` documents keep their meaning.
