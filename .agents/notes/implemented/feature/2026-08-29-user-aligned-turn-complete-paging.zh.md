...
- 两条读路径对同一游标产出完全一致的页边界，边界语义只有一个家（`nthMessageCut`）；快路径与观察路径不再可区分。
- 页从回合开口开始并携带组头事件，Turn Process 折叠在每页首绘即正确——Load earlier 的闪烁消失。
- 压缩相邻页重新走上索引快路径（4096 事件的深层重试吸收加宽的切点）；无索引切点的存储仍回退观察路径。

## 备选方案

- **分别修两条路径**（快路径保留消息种类锚点）：分裂语义本身就是缺陷；此后每条边界规则都要编码两次。
- **无论回合状态如何都按页边界供头**：已折叠的头渲染错位；加宽到回合开口才让边界稳定。
- **加大前导边距代替深层重试**：更宽的前导惩罚所有索引读，只为覆盖压缩这一种情形；重试梯子只在切点确实需要时付费。
