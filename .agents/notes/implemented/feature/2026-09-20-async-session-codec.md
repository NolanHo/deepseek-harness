...
**The pool is awaited only after the read transaction returns.** `loadStoredLog` and `loadStoredFrom` select their rows inside a synchronous `readTransaction` and hydrate afterwards. One `DatabaseSync` connection cannot carry two open transactions, and an await inside one opens the window: another operation's `BEGIN` fails with `cannot start a transaction within a transaction`, and its `ROLLBACK` ends the transaction the writer still believes it holds — every row written before that rollback is discarded, a row written after it autocommits outside any transaction, and the writer's `COMMIT` then fails with `cannot commit - no transaction is active`. Every write path therefore keeps the synchronous decoder even with the switch on.
...
**Await the pool inside the read transaction.** Rejected: `readTransaction(async () => …)` is the natural shape, and what it produces is the transaction hazard above rather than a slowdown — a nested `BEGIN` that cannot start, a `ROLLBACK` from another operation that discards the writer's rows, and a writer's `COMMIT` failing afterwards.
