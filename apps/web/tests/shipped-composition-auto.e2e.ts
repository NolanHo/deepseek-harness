// Shipped-composition scenarios for the Auto producer-to-tool path: one
// browser-authored request, one-shot/continuable/cold-resumed child calls,
// failed initialization rollback, and Loader-unload withdrawal.
//
// Fork note (FORK_SURFACE.md, test updates): every case here drives the
// `permissionPresets` service to switch a Session into Auto, and this fork
// disables the `permission` row in packages/bundle/base/cordis.patch.yml, so
// the shipped composition mounts no permission service and the `auto-review`
// entry stays pending on it. The Web lane excludes this file for that
// composition reason; restore path: re-enable the `permission` row and drop
// the file's entry from vitest.web.config.ts.
